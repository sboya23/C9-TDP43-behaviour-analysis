---
title: "C9orf72 mice - elevated plus maze analysis"
...
data_mouse <- read_csv("C9_EPM_standard_harmonised.csv", col_types = cols (
  animal_name = 'f',
  genotype = 'f',
  sex = 'f',
  inh = 'f',
...
ME_EXPL_D<- lmer(Duration_sec~genotype*(sex + inh + Section) + (1|animal_name),data = data_mouse)
summary(ME_EXPL_D)
...
ME_EXPL_D_lm<- lm(Duration_sec~genotype*(sex + inh + Section), data = data_mouse)
...
data_mouse$genotype <- relevel(data_mouse$genotype, ref = "C9ORF72-GR400-B6J:WT")
#linear mixed effects model 
ME_EXPL_F<- lmer(Frequency~genotype*(sex + inh + Section) + (1|animal_name), data = data_mouse)
summary(ME_EXPL_F)
...
custom_contrasts <- list("Wt vs Het closed" = c(1, -1, 0, 0, 0, 0),
                         "Wt vs Het open" = c(0, 0, 1, -1, 0, 0),
                         "Wt vs Het centre" = c(0, 0, 0, 0, 1, -1))
...
***Plot exploration duration by Section:***
```{r}
...
EXPL_EPM_D$genotype <- factor(EXPL_EPM_D$genotype, levels = c("C9ORF72-GR400-B6J:WT", "C9ORF72-GR400-B6J:Het"))
...
pEXPL_EPM_D <- ggplot(EXPL_EPM_D, aes(x = genotype, y = mean, group = Section, colour = genotype, shape = Section)) +
  ...
  scale_x_discrete(labels = c(
    ...
    "C9ORF72-GR400-B6J:Het" = expression(bolditalic("C9orf72")^bolditalic("GR400/+"))
  ...
  scale_colour_manual(
    ...
    labels = c("WT", expression(italic("C9orf72")^italic("GR400/+"))),
...
# Save the plot
ggsave("C9_EPM_dur_av_Dec24-arial.tiff", plot = pEXPL_EPM_D, width = 3.5, height = 3, dpi = 600)
...
# Ensure the factor levels are ordered as desired
EXPL_EPM_F$genotype <- factor(EXPL_EPM_F$genotype, levels = c("C9ORF72-GR400-B6J:WT", "C9ORF72-GR400-B6J:Het"))
...
# Save the plot
ggsave("C9_EPM_freq_av_Dec24-arial.tiff", plot = pEXPL_EPM_F, width = 3.5, height = 3, dpi = 600)
