...
data_mouse <- read_csv("output_long_weight_C9_averaged_multiples_n3.csv", col_types = cols (
  animal_name = 'f',
  genotype = 'f',
  sex = 'f',
  inh = 'f',
...
ME_EXPL<- lmer(averageweight~genotype*(age_weeks + sex + inh) + age_weeks*sex + (1|animal_name),data = data_mouse)
summary(ME_EXPL)
...
for (age in age_levels) {
  ...
  pairwise_comparisons <- contrast(emms_age, method = "pairwise", by = "genotype")
...
***Plot - weight - C9orf72 by genotype and age***
```{r}
...
display_levels <- levels(EXPL_weight$age_weeks)[seq(1, length(levels(EXPL_weight$age_weeks)), by = 9)]
...
pEXPL_weight <- ggplot(EXPL_weight, aes(x = age_weeks, y = mean, group = genotype_sex, linetype = sex, colour = genotype)) +
 # geom_point(size=0.4) + 
  ...
  scale_colour_manual(name = NULL, # Customize the color legend
    breaks = c("C9ORF72-GR400-B6J:WT", "C9ORF72-GR400-B6J:Het"), # Order of items in legend
    labels = c("WT", expression(italic("C9orf72")^italic("GR400/+"))), # Labels in legend
    values = c("#009E73", "#332288") # Colors for genotypes
  ) +
...
# Save the plot
ggsave("C9_weight_linetype_colour_600_legend-arial.tiff", width =3.5, height = 3, dpi = 600)  # Save as an image
